...
for $doc in collection(concat($DIR,"/?select=*.xml")) (: "/home/aardvark/corp/nl/SONAR_ccl/WRPPG_3?select=*.xml" :)
    for $article in $doc//article.published
      ...
      then  <pre>
            ...
            <author>{if ($article//namefirst/text() or $article//namelast/text()) then (string-join($article//personname/descendant::*/text()," "))
                     else (string-join($article//authortagline//sentence/text(),","))}</author>
            <topic>{lower-case($article//topic/text())}</topic>
            <section>{lower-case($article//section/text())}</section>
            <subsection>{lower-case($article//subsection/text())}</subsection>
            <edition>{string-join($article//edition/text(),",")}</edition>
            <date>{if (substring($article//story.date/text(),1,10)) then (substring($article//story.date/text(),1,10))
                   else (substring($article/head/id/@pubdate,1,10))}</date>
            {$node/../sentence,
